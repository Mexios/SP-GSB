...
class PdoGsb{   		
      	private static $serveur='oci:dbname=192.168.0.42:1521/xe';
      	//private static $bdd='dbname=gsbV2';   		
      	private static $user='mexios' ;    		
      	private static $mdp='maxime67' ;	
	...
	public function majEtatFicheFrais($idVisiteur,$mois,$etat){
        ...
        public function RecuperationLogin()
        {
            ...
            $Resultat = PdoGsb::$monPdo->query($req);
                ...
                {
                    $LesLogins[] = $ligne['LOGIN'];
                }
           return $LesLogins;     
        }
        
        
        public function SuppressionUtilisateur($unUtilisateur)
        {
        ...
        public function RecupererInfosUtilisateur($unUtilisateur)
        {
            $unUtilisateur = $_SESSION['Utilisateur'];
            $req = "Select * from Utilisateur where Login='".$unUtilisateur."'";
            $Resultat = PdoGsb::$monPdo->query($req);
            $ligne = $Resultat->fetch();
            $_SESSION['IdUtilisateur'] = $ligne['ID'];
            $_SESSION['NomUtilisateur'] = $ligne['NOM'];
            $_SESSION['PrenomUtilisateur'] = $ligne['PRENOM'];
            $_SESSION['LoginUtilisateur'] = $ligne['LOGIN'];
            $_SESSION['PasswordUtilisateur'] = $ligne['MDP'];
            $_SESSION['AdresseUtilisateur'] = $ligne['ADRESSE'];
            $_SESSION['CodePostalUtilisateur'] = $ligne['CP'];
            $_SESSION['VilleUtilisateur'] = $ligne['VILLE'];
            $_SESSION['DateUtilisateur'] = $ligne['DATEEMBAUCHE'];
        ...
        public function RecupererDateMdp($unLogin,$unMdp)
        {
            $unMdp = md5($unMdp);
            $req = "Select datemdp from Utilisateur where Login='".$unLogin."' and mdp='".$unMdp."'";
            $Resultat = pdoGsb::$monPdo->query($req);
            $ligne = $Resultat->fetch();
            $_SESSION['AncienneDate'] = $ligne['DATEMDP'];
        }
        
        public function UpdateMotDePasse($unLogin,$unMdp)
        {
            $req = "update Utilisateur set mdp='".$unMdp."' where Login='".$unLogin."'";
            pdoGsb::$monPdo->exec($req);
        }
}
?>